...
const mockCategoriesByMap: Record<string, CategoryDto[]> = {
    "map-1": [
        {
...
const getCategories = (mapId: string): CategoryDto[] => {
    if (!mockCategoriesByMap[mapId]) {
        mockCategoriesByMap[mapId] = [];
    }
    return mockCategoriesByMap[mapId];
};

const setCategories = (mapId: string, categories: CategoryDto[]) => {
    mockCategoriesByMap[mapId] = categories;
};

const mockMapPointsByMap: Record<string, MapPointDto[]> = {
    "map-1": [
        {
...
const getMapPoints = (mapId: string): MapPointDto[] => {
    if (!mockMapPointsByMap[mapId]) {
        mockMapPointsByMap[mapId] = [];
    }
    return mockMapPointsByMap[mapId];
};

const setMapPoints = (mapId: string, points: MapPointDto[]) => {
    mockMapPointsByMap[mapId] = points;
};
...
export const handlers = [
    ...
    http.get(
        ...
        async ({ params }) => {
            await delay(1000);
            const { mapId } = params;
            const categories = getCategories(mapId as string);
    ...
    http.post(
        ...
        async ({ params, request }) => {
            ...
            setCategories(mapId as string, [...currentCategories, category]);
            categoryIdCounter++;
